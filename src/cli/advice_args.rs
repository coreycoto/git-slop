...
struct AdviseArgs {
    ...
    #[arg(long, conflicts_with_all = ["context_only", "validate_artifact"])]
    infer: bool,
    ...
    #[arg(long, value_enum, requires = "infer", conflicts_with = "validate_artifact")]
    provider: Option<crate::advice::ProviderKind>,
    /// Explicit loopback provider endpoint. Remote endpoints are refused in V1.
    #[arg(long, requires = "infer", conflicts_with = "validate_artifact")]
    ...
    #[arg(long, value_enum, default_value_t = crate::advice::ReasoningEffort::Medium, requires = "infer", conflicts_with = "validate_artifact")]
    reasoning: crate::advice::ReasoningEffort,
    /// Provider connection timeout in seconds.
    #[arg(long, default_value_t = 5, value_parser = clap::value_parser!(u64).range(1..=60), requires = "infer", conflicts_with = "validate_artifact")]
    connect_timeout_seconds: u64,
    /// Total model-load and generation timeout in seconds.
    #[arg(long, default_value_t = 120, value_parser = clap::value_parser!(u64).range(1..=3600), requires = "infer", conflicts_with = "validate_artifact")]
    timeout_seconds: u64,
    /// Maximum accepted provider response size in bytes.
    #[arg(long, default_value_t = 1_048_576, value_parser = parse_max_response_bytes, requires = "infer", conflicts_with = "validate_artifact")]
    max_response_bytes: usize,
    /// Maximum generated output tokens requested from the provider.
    #[arg(long, default_value_t = 2048, value_parser = parse_max_output_tokens, requires = "infer", conflicts_with = "validate_artifact")]
    max_output_tokens: usize,
    /// Total provider context window. Defaults to the input and output token budgets combined.
    #[arg(long, value_parser = parse_runtime_context_tokens, requires = "infer", conflicts_with = "validate_artifact")]
