...
The examples below pin 0.11.6. Confirm that the requested distribution surface
...
After the bucket lists 0.11.6, install it from PowerShell:
...
```bash
cargo install git-slop --version 0.11.6 --locked
git-slop build-info --format json
```

For a verified 0.11.6 release, `source_revision` is the full commit named by
`v0.11.6` and `source_dirty` is `false`. A local source build can report `null`
