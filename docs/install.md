...
The examples below pin 0.16.1. Confirm that the requested distribution surface
...
```bash
cargo binstall git-slop@0.16.1
git-slop version
git-slop build-info --format json
```
...
```bash
cargo binstall --force git-slop@0.16.1
...
After the bucket lists 0.16.1, install it from PowerShell:
...
```bash
gh release verify v0.16.1 --repo coreycoto/git-slop
```
...
schema-3 manifest contains these exact commands; for `v0.16.1` they are:

```bash
gh attestation verify git-slop-v0.16.1-aarch64-apple-darwin.tar.gz --repo coreycoto/git-slop
gh attestation verify git-slop-v0.16.1-aarch64-pc-windows-msvc.zip --repo coreycoto/git-slop
gh attestation verify git-slop-v0.16.1-aarch64-unknown-linux-gnu.tar.gz --repo coreycoto/git-slop
gh attestation verify git-slop-v0.16.1-x86_64-apple-darwin.tar.gz --repo coreycoto/git-slop
gh attestation verify git-slop-v0.16.1-x86_64-pc-windows-msvc.zip --repo coreycoto/git-slop
gh attestation verify git-slop-v0.16.1-x86_64-unknown-linux-gnu.tar.gz --repo coreycoto/git-slop
gh attestation verify git-slop-v0.16.1-x86_64-unknown-linux-musl.tar.gz --repo coreycoto/git-slop
```
...
```bash
cargo install git-slop --version 0.16.1 --locked --force
cargo uninstall git-slop
```

For a verified 0.16.1 release, `source_revision` is the full commit named by
`v0.16.1` and `source_dirty` is `false`. A local source build can report `null`
