...
class DetectorIntegrationTests(unittest.TestCase):
    def test_find_generates_report_without_prior_init(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("costs", report["files"][0])
            self.assertIn("overlays", report["files"][0])
            self.assertIn("load", report["files"][0]["costs"])
            self.assertIn("organization_health", report["files"][0]["overlays"])
            ...
            self.assertIn("Overlay Highlights", summary_contents)
    ...
    def test_show_and_check_use_generated_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("costs", folder_record)
            self.assertIn("overlays", folder_record)
