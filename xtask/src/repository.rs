...
pub fn validate_overlays(repo_root: &Path) -> Vec<String> {
    ...
    if let Some(palette) = palette {
        ...
        match palette.pointer("/labels").and_then(Value::as_array) {
            Some(labels) => {
                for (index, label) in labels.iter().enumerate() {
                    ...
                    if label.get("owner").and_then(Value::as_str) == Some("repo-managed") {
                        match label.get("name").and_then(Value::as_str) {
...
fn load_json(path: &Path, label: &str, errors: &mut Vec<String>) -> Option<Value> {
...
mod tests {
    ...
    fn repository_overlays_pass() {
        ...
        assert_eq!(validate_overlays(root), Vec::<String>::new());
    }
    ...
    fn malformed_required_entries_fail_closed() {
        ...
        let errors = validate_overlays(root);
