...
- the local marketplace that publishes the `git-slop` product plugin
...
`git-slop` also publishes `git-slop@git-slop-marketplace` from
`plugins/git-slop`. That plugin owns product-specific guidance for installing,
running, interpreting, planning from, and adopting the `git-slop` CLI. It should
